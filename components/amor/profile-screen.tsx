...
export function ProfileScreen({ onOpenSettings, onOpenShop, onOpenEdit }: ProfileScreenProps) {
  ...
  return (
    <div className="flex h-[100dvh] flex-col bg-background overflow-hidden">
      ...
      <div
        className={cn(
          "fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 transition-all duration-300 pointer-events-none",
          scrolled ? "glass-strong py-2 border-b border-white/5" : "bg-transparent py-3"
        )}
        style={{ paddingTop: scrolled ? "calc(env(safe-area-inset-top, 0px) + 8px)" : "calc(var(--topbar-h) - 40px)" }}
      >
        <button
          onClick={onOpenSettings}
          className={cn(
            "flex h-9 w-9 items-center justify-center rounded-xl glass active:scale-95 transition-all pointer-events-auto",
            scrolled && "bg-white/5"
          )}
        >
        ...
        <button
          onClick={onOpenShop}
          className={cn(
            "flex items-center gap-1.5 rounded-full glass px-3 py-1.5 active:scale-95 transition-all pointer-events-auto",
            scrolled ? "bg-white/5 border-white/10" : "border-amor-gold/20"
          )}
        >
      ...
      <div className="flex-1 overflow-y-auto pb-20" onScroll={handleScroll}>
        <div className="relative shrink-0" style={{ marginTop: "calc(var(--topbar-h) - 56px)" }}>
